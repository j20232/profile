...
function init(){
  ...
  function vibrate(){
    ...
    for(var j = 0; j < particleHolder.length; j++){
      ...
      particleMouse = Math.max( Math.min( 35 / ( particleDistance /p.rad ), 10 ), 1.0 );
      context.beginPath();
      context.fillStyle = p.color;
      angleX += 0.0001 * Math.random()
      angleY += 0.0001 * Math.random()
      context.arc(p.x + Math.sin(angleX*Math.cos(radius)), 
      p.y - Math.cos(angleY*Math.sin(radius)), 
